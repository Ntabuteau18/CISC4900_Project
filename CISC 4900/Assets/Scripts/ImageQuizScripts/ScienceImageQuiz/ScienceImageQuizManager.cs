...
public class ScienceImageQuizManager : MonoBehaviour
{
    /**
    ...
    * 
    * */
    public List<ScienceImageQnA> QnA;
    ...
    void SetAnswers()
    {
        ...
        {
            options[i].GetComponent<ScienceImageAnswers>().isCorrect = false;
            options[i].transform.GetChild(0).GetComponent<TMP_Text>().text = QnA[currentQuestion].Answers[i];

            if (QnA[currentQuestion].CorrectAnswer == i + 1)
            {
                options[i].GetComponent<ScienceImageAnswers>().isCorrect = true;
            }
        }
    }
